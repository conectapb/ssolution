...
else if ($modo=="alt" || $modo=="vis")
{
	...
	if($modo=="vis")
	{
		echo utf8_encode($visual->fetch('adm/grupos_visualizar.tpl'));
	}
	else
	{
		$visual->assign('subtitulo','Alteração de Grupo');
...
{
	$dados = sql2array(
		"SELECT COUNT(C.id) AS n_clientes,A.*,B.nome AS grupo_tipo
		FROM grupos A 
		INNER JOIN grupos_tipos B ON A.tipo = B.id
		LEFT JOIN clientes C ON C.grupo_id = A.id
		GROUP BY A.id
