...
function rootpath_dir() {
	...
	$rootpath_dir=add_trailing_slash(getcwd(),SLASH); //save this, add a slash (if it needs one)
...
function perlpath () {
	if ((isset($_ENV['OS'])) && ($_ENV['OS'] == 'Windows_NT')) {
		...
		foreach ($paths as $path) {
			$files[]=$path.'\perl\bin\perl.exe';
			$files[]=$path.'\perl.exe';
		}
		$files[]='C:\perl\bin\perl.exe';
		foreach ($files as $file) {
			if (file_exists($file)) { $perlpath=$file; }
		}
		return $perlpath;
	}
	else {
		$perlpath=`which perl`;
		if (!$perlpath) { $perlpath='/usr/local/bin/perl'; }
	}
	return $perlpath;
}
...
function mysql_run_queries($sql_query,$VERBOSE=0) {
				...
				//if($sql_query != "") {
					...
					for($i = 0; $i < $sql_count; $i++)
					{
						...
						if(!empty($sql) and $sql[0] != "#")
						{
							...
							if (!$result) { echo 'Warning: '.mysql_error()."<br>\n"; $error=mysql_error(); }
						}
					}
...
function mysql_restore_db($file,$VERBOSE=0) {
		$sql_query=file_get_contents($file);
		mysql_run_queries($sql_query,$VERBOSE);
}
